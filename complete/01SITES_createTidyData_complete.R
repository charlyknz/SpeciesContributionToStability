...
zoo.data <- zoo %>%
  ...
  select(Lake, Experiment, Treatment, Sampling, Exp_day, Replicate, Taxa, Abundance,Clean_Biomass)%>%
  group_by(Lake, Experiment)%>%
  complete(Treatment, Sampling, Taxa, Replicate, fill = list(Biomass = 0)) %>% # have Biomass which is NA as 0 to calculate the correct mean here 
...
Sva <- zoo.data %>%
  filter(Lake == 'Sva' & Replicate== 1 & Sampling == 6)
unique(Sva$Taxa)

Erk <- zoo.data %>%
  filter(Lake == 'Erk' & Replicate== 1 & Sampling == 6)
unique(Erk$Taxa)
### ###
...
dum<-zoo.data %>% 
  filter(Treatment != 'Lake') %>%# reduce dataset to experimental dataset
  group_by(Lake) %>%
  complete(Experiment, Treatment, Sampling, Taxa, Replicate) %>%
  group_by(Lake, Experiment, Treatment, Sampling, Taxa) %>% 
