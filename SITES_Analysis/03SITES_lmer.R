...
#### read data ####
data<-read.csv2("~/Desktop/phD/SpeciesContributionToStability/SITES_Data/AUCdata_biom.csv")
names(data)
...
mod5.presspulse.aucRR<-lmer(AUC.RR~Taxa-1 + (1|Lake) + (1|Experiment),
                       data=data[data$Treatment=="Pulse_Press",])
...
tab_model(mod1.press.aucRR,mod2.press.aucPI,mod3.pulse.aucRR,
          ...
          mod6.presspulse.aucPI,digits = 4, show.ci = FALSE, file = "YOURTABLENAME.doc")
