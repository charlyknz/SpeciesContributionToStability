...
#### size ####
## Note: run Script 1 first!!!
zoo.size <- zoo %>%
  mutate(size = Mean_lenght*1000) %>% #convert size into um
  group_by(Zooplankton_group, Lake,Experiment, Treatment, Taxa ) %>%
  summarise(mean.size = mean(size, na.rm = T),
            sd.size = sd(size, na.rm = T)) %>%
  arrange(Treatment, desc(mean.size)) %>%
  filter(!Taxa %in% c('small Cladoceran', 'Nauplii'))
names(zoo.size)

write.csv2(zoo.size, file = here('output/zoo.size.neu.csv'))

zoo.size1 <- read_csv2("output/zoo.size.neu.csv") %>% 
  select(-...1  ) %>%
...
zoo.size1$Treatment[zoo.size1$Treatment == "F" ]<- "Pulse"
zoo.size1$Treatment[zoo.size1$Treatment == "FS"] <- "Pulse & Press"
zoo.size1$Treatment[zoo.size1$Treatment == "S"] <- "Press"

zoo.size1$Experiment[zoo.size1$Experiment == 1] <- 'spring'
zoo.size1$Experiment[zoo.size1$Experiment == 2] <- 'summer'

zoo.size1[zoo.size1$Lake == "Asa", "Lake"] <- "Feresjörn"
zoo.size1[zoo.size1$Lake == "Erk", "Lake"] <- "Erken"
zoo.size1[zoo.size1$Lake == "Sva", "Lake"] <- "Stortjärn"
zoo.size1[zoo.size1$Lake == "Sko", "Lake"] <- "Erssjön"
zoo.size1[zoo.size1$Lake == "Bol", "Lake"] <- "Bolmen"

### join size with relat.dom data ###
zoo.with.size <- left_join(Raw.dom.zoo, zoo.size1, by = c("Taxa", 'Lake', 'Experiment' )) #%>%
names(zoo.with.size)
unique(zoo.with.size$Taxa)
hist(log(zoo.with.size$mean.size))

corrPlot <- ggscatter(zoo.with.size, y = 'mean.size', x = 'relat.dom',ylab = 'Mean size (in µm)', xlab = 'Mean relative dominance',add = 'reg.line', conf.int = T)  +
  stat_cor( label.x = 0.1) 
corrPlot


AUC.pi.size <- ggplot(zoo.with.size,  aes(x = mean.size, y=mean.AUC.pi, color = mean.trend.pi))+
  geom_point(alpha = .5)+
  scale_color_manual(values = c('#D56060', 'grey','#68789E'))+
 # scale_x_log10(limits = c(40, 3200),breaks = c(100, 300, 1000, 3000))+
  geom_hline(yintercept = 0, alpha = 0.5) +
  labs(x = "Size (in µm)", y = 'Relative contribution to stability') +
  theme_bw() +
  facet_grid(~Treatment)+
  theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank()) +
  theme(legend.title = element_blank(),
...
AUC.rr.size <- ggplot(zoo.with.size,  aes(x = mean.size, y=mean.AUC.RR, color = mean.trend.RR))+
  geom_point(alpha = .5)+
  scale_color_manual(values = c('#D56060', 'grey','#68789E'))+
 # scale_x_log10(limits = c(40, 3200),breaks = c(100, 300, 1000, 3000))+
  geom_hline(yintercept = 0, alpha = 0.5) +
  labs(x = "Size (in µm)", y = 'Absolute contribution to stability') +
  theme_bw() +
  facet_grid(~Treatment)+
  theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank()) +
  theme(legend.title = element_blank(),
...
plot_grid(AUC.rr.size, corrPlot, AUC.pi.size, ncol = 2,hjust = -0.2,labels = c('(a)','(c)', '(b)', '(d)'),rel_widths = c(2.5,1.5))
